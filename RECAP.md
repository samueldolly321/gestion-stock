...
- `desktop/` — **client Electron** : `main.js` (fenêtre + mémorisation de l'URL du serveur + gestion « serveur injoignable » + **autorisation caméra `setPermissionRequestHandler('media')` pour le scan par webcam**), `preload.js` (pont sécurisé), `renderer/config.html` (saisie de l'adresse du serveur au 1er lancement). Build via **electron-builder** : `npm run dist` → installeur Windows NSIS (`.exe` ~78 Mo, **version courante `1.1.0`**) ; `npm run dist:portable` → version portable `.zip`. Sortie dans `desktop/dist-installer/` (ignoré par git). **Rebuild nécessaire uniquement pour un changement de `main.js`** — les évolutions de l'app arrivent par le serveur.
